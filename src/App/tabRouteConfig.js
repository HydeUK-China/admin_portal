...
function path_name_component(role, uid) {
    return {
        ...
        expert_profile: { path: '/mgt/expert_profile', name: 'Profile', icon: 'fa fa-user ', component: <ExpertProfile role={role} uid={uid} /> },
...
export function path_name(role, uid){
    const role_objects = path_name_component(role, uid);
    const pick_tabs = _.pick(role_objects, role_pages[role]);

    return pick_tabs;
}

export function renderRoute(role, uid) {
    const role_objects = path_name_component(role, uid);
